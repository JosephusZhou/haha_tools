...
class HarmonyResPage extends StatefulWidget {
  const HarmonyResPage({Key? key}) : super(key: key);

  @override
  State<HarmonyResPage> createState() => _HarmonyResPageState();
}

class _HarmonyResPageState extends BaseState<HarmonyResPage>
    implements DragContainerListener {

  final List<String> _hosDpiList = [
    "mdpi",
    "ldpi",
    "xldpi",
    "xxldpi",
  ...
  final List<String> _originalDpiList = [
    "-mdpi",
    "-hdpi",
    "-xhdpi",
    "-xxhdpi",
  ...
  String _resDir = AppConfig().readHarmonyResDir();

  final List<bool> _dpiCheckList = [false, true, true, true, true];
                        ...
                        Expanded(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              divMargin4,
                              radioWidget("dark", _dayNightSuffix, (value) {
                                setState(() {
                                  _dayNightSuffix = value!;
                                });
                              }),
                              divMargin4,
                              radioWidget("light", _dayNightSuffix, (value) {
                                setState(() {
                                  _dayNightSuffix = value!;
                                });
                              }),
  ...
  List<Widget> buildDpiCheckBoxWidget(Widget marginWidget) {
    ...
    for (var i = 0; i < _hosDpiList.length; i++) {
      list.add(marginWidget);
      list.add(checkWidget(_dpiCheckList[i], _hosDpiList[i], (value) {
        setState(() {
          _dpiCheckList[i] = value!;
        });
      }));
    }
    return list;
  ...
  void selectDir() async {
    ...
    if (dir != null) {
      ...
      AppConfig().writeHarmonyResDir(dir);
    }
  }
  ...
  void getFiles(String? filePath) {
    if (filePath != null) {
      ...
      // 获取同级根目录的文件
      String selectedDpi = "";
      for (var mipDpi in _originalDpiList) {
  ...
  void copyFileAndDoCommand(BuildContext context) async {
    ...
    for (var i = 0; i < _hosDpiList.length; i++) {
      var hosDpi = _hosDpiList[i];
      var dpi = _originalDpiList[i];
      ...
      if (_dpiCheckList[i] && originalFilePath.isNotEmpty) {
        // zh-rTW-dark-mdpi
        String dpiDirName = "";
